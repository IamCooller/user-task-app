...
const fakeTasks: Task[] = [generateTask({ uuid: "3", completed: false }), generateTask({ uuid: "4", completed: false })];
...
describe("ListComponent", () => {
	...
	beforeEach(() => {
		TestBed.configureTestingModule({
			imports: [BrowserModule, BrowserAnimationsModule, FormsModule, ReactiveFormsModule, MatCardModule, MatChipsModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatIconModule],
	...
	it(`should display total number of tasks`, () => {
		...
		expect(total.nativeElement.textContent).toEqual(`Total Tasks: ${fakeTasks.length}`);
	});
	...
	it(`should mark a task as complete when done button is clicked`, async () => {
		...
		const doneButton = await loader.getHarness(MatButtonHarness.with({ selector: '[data-testid="complete-task"]' }));
	...
	it(`should mark a task as archived when delete button is clicked`, async () => {
		...
		const deleteButton = await loader.getHarness(MatButtonHarness.with({ selector: '[data-testid="delete-task"]' }));
	...
	it("should not display archived tasks after deleting them", async () => {
		let displayedCards = fixture.debugElement.queryAll(By.css("mat-card"));
		expect(displayedCards.length).toEqual(fakeTasks.length);

		jest.spyOn(tasksService, "getTasksFromStorage").mockImplementation(() => {
		...
		await tasksService.getTasksFromStorage();
		fixture.detectChanges();

		displayedCards = fixture.debugElement.queryAll(By.css("mat-card"));
		expect(displayedCards.length).toEqual(fakeTasks.length - 1);
	});
});
