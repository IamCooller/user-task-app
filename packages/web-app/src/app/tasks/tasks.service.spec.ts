...
describe("TasksService", () => {
	...
	beforeEach(() => {
		TestBed.configureTestingModule({
			...
			providers: [TasksService, { provide: StorageService, useClass: MockStorageService }],
		});
	...
	describe("filterTask", () => {
		...
		it("should filter task by priority key", () => {
			service.tasks = [generateTask({ priority: TaskPriority.LOW }), generateTask({ priority: TaskPriority.HIGH })];
			service.filterTask("priority");
			expect(service.tasks.length).toEqual(1);
		});

		it("should filter task by completed key", () => {
			service.tasks = [generateTask({ completed: false }), generateTask({ completed: true })];
		...
		it("should filter task by scheduledDate key", () => {
			...
			const taskToday = generateTask({ scheduledDate: today });
			const taskTomorrow = generateTask({ scheduledDate: tomorrow });

			service.tasks = [taskToday, taskTomorrow];

			service.filterTask("scheduledDate");

			expect(service.tasks.length).toEqual(1);
			expect(new Date(service.tasks[0].scheduledDate).toDateString()).toEqual(today.toDateString());
		});
	});

	describe("searchTask", () => {
		it("should search task list for title with search term", () => {
			service.tasks = [generateTask({ title: "Take home assignment" }), generateTask({ title: "Thank you for your time" })];
